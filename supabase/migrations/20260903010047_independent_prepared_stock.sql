-- Prepared selling units are authoritative; components remain an audit ledger.
create or replace function private.validate_selling_unit_stock()
returns trigger language plpgsql security invoker set search_path = ''
as $$
declare item record; brought integer; already_sold bigint;
begin
  if auth.uid() is null or not private.is_business_member(new.business_id,auth.uid()) then
    raise exception 'You do not have access to this business';
  end if;
  -- Serialize prepared-unit validation with sales and removals for this day.
  perform 1 from public.selling_days where id=new.selling_day_id
    and business_id=new.business_id for update;
  if not found then raise exception 'Selling day not found'; end if;
  for item in
    select variant_id,sum(quantity) as quantity
    from jsonb_to_recordset(new.items) as i(variant_id bigint,quantity integer)
    group by variant_id order by variant_id
  loop
    select brought_quantity into brought from public.selling_day_variants
      where selling_day_id=new.selling_day_id and variant_id=item.variant_id for update;
    select coalesce(sum(si.quantity),0) into already_sold from public.sale_items si
      join public.sales s on s.id=si.sale_id
      where s.selling_day_id=new.selling_day_id and s.voided_at is null
        and si.variant_id=item.variant_id;
    if brought is null or item.quantity is null or item.quantity < 1
      or item.quantity > brought-already_sold then
      raise exception 'Not enough prepared stock for this selling option';
    end if;
  end loop;
  return new;
end;
