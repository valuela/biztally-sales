create function public.remove_prepared_stock(
  p_selling_day_id bigint, p_variant_id bigint, p_quantity integer, p_kind text
) returns void
language plpgsql security invoker set search_path = ''
as $$
declare
  b bigint; brought integer; sold bigint; component record; available bigint;
begin
  ...
  if p_quantity is null or p_quantity < 1 or p_kind is null
    or p_kind not in ('returned_home','damaged','giveaway') then
  ...
  loop
    select ds.brought_quantity
      + coalesce((select sum(a.quantity_delta) from public.stock_adjustments a
          where a.selling_day_id=p_selling_day_id and a.variant_id=component.id),0)
      - coalesce((select sum(sc.quantity) from public.sale_item_components sc
          join public.sale_items si on si.id=sc.sale_item_id
          join public.sales s on s.id=si.sale_id
          where s.selling_day_id=p_selling_day_id and s.voided_at is null
            and sc.component_variant_id=component.id),0)
    into available from public.daily_stock ds
    where ds.selling_day_id=p_selling_day_id and ds.variant_id=component.id for update;
    if available is null or available < component.quantity::bigint*p_quantity then
      raise exception 'Not enough remaining stock. Refresh and try again.';
    end if;
