...
-- Preserve historical records without guessing how customers paid.
update public.sales set payment_method = 'unknown' where payment_method is null;
update public.sale_commands set payment_method = 'unknown' where payment_method is null;
...
alter table public.sale_commands
  alter column payment_method set not null,
  add constraint sale_commands_payment_method_check
    check (payment_method in ('cash', 'gcash', 'bank_transfer', 'unknown'));
